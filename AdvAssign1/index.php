<?php

    require 'ProcessApi.php';
    $url = 'https://www.innoraft.com/jsonapi/node/services';
    $newProcess = new ProcessApi($url);
    $newProcess->process();
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Advance 1</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <div class="container">
        <?php for($i=0; $i<count($newProcess->title); $i++) {?>
            <div class="services <?php if($i%2==0) echo "odd"; ?>">
                <div class="title">
                    <?php echo $newProcess->title[$i];?>
                    <div class="icons">
                        <?php for($j=0; $j<count($newProcess->icons[$i]); $j++){?>
                            <img class="service-icon" src="<?php echo $newProcess->icons[$i][$j];?>" alt="">
                        <?php }?>
                    </div>

                    <div class="services-desc">
                        <?php echo $newProcess->desc[$i]; ?>
                        <a class="explore" href="<?php echo $newProcess->explore[$i]; ?>"><strong>Explore</strong></a>
                    </div>
                </div>
                <div class="image">
                    <img src="<?php echo $newProcess->main_img[$i]?>" alt="">
                </div>
            </div>
                <?php
            }?>
    </div>
</body>
</html>